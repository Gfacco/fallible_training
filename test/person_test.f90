...
contains
  ...
  function check_missing_location_key() result(result_)
    type(result_t) :: result_
    type(fallible_person_t) :: maybe_person
    type(error_list_t) :: errors
        character(len=*), parameter :: person_no_location_key = &
       '{                                       ' // NEWLINE &
    // '    "name" : 2.0            ' // NEWLINE &
    // '} '

    maybe_person = fallible_person_t(parse_json_from_string(person_no_location_key))
    errors = maybe_person%errors()
    if (maybe_person%failed()) then
      result_ = assert_that((errors.ofType.NOT_FOUND).hasAnyIncluding."location", errors%to_string(), &
